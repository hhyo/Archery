...
def execute(workflow_id,user_username=None):
    ...
    execute_result = execute_engine.execute_workflow(workflow=workflow_detail).to_dict()
...
@shared_task
def execute_callback(result,workflow_id):
    """异步任务的回调, 将结果填入数据库等等
    ...
    workflow.finish_time = result['end_time']
    # 查询特定任务ID的结果
    task_result = result['execute_result']
    # 检查所有任务是否执行成功
    all_tasks_successful = all(task['errlevel'] == 0 for task in task_result)
    ...
    try:
        # 保存执行结果
        workflow.sqlworkflowcontent.execute_result = json.dumps(task_result)
