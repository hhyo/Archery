...
class ExecuteCheck(views.APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            max_sql_length = SysConfig().get("max_sql_length", default_value=0)
            if (len(check_result.rows) > int(max_sql_length)) and not max_sql_length:
                raise serializers.ValidationError(
                    {"errors": f"执行行数超过{str(max_sql_length)}行，请联系管理员核查！"}
                )
