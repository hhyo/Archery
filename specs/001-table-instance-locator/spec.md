...
1. **Given** 用户在查询页面右侧面板实例选择器上方的输入框中输入表名，**When** 点击定位按钮，**Then** 在同区域内展示以 `实例名/数据库名/表名` 格式拼接的单列匹配结果列表，用户可据此手动选择目标实例。
...
- **FR-011**: 前端 MUST 以 `实例名/数据库名/表名` 单列格式在输入框下方展示匹配结果列表；无匹配时展示明确的无匹配提示；调用失败时展示可读错误信息。
- **FR-012**: 前端输入框内容清空时，MUST 取消待发请求并清空结果列表，不调用后端接口。- **FR-002**: 系统 MUST 校验请求对象结构与字段约束；当输入不满足规则时返回明确错误，不进入定位流程。
...
- Q: 查到结果后，用户点击某条记录，前端应如何响应？ → A: 仅高亮展示结果列表，用户手动选择
