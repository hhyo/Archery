...
- [X] T046 [US3] Add list-link fallback logic in sql/templates/sqlworkflow.html, sql/templates/sqlexportworkflow.html, sql/templates/queryapplylist.html, and sql/templates/archive.html so historical rows without `audit_id` use legacy detail URLs instead of `/workflow/None/`
- [X] T047 [US3] Update template tests in sql/tests.py to assert unified links are used when `audit_id` exists and legacy detail fallback branches exist when `audit_id` is missing
- [X] T048 [US1] Verify normal creation transaction behavior for SQL Workflow, query privilege, and archive flows: audit creation failure rolls back the new work order instead of leaving a visible no-audit work order
...
- `.venv/bin/python -m pytest sql/tests.py::TestView::test_sqlworkflow sql/tests.py::TestView::test_sqlexportworkflow sql/tests.py::TestView::test_queryapplylist sql/tests.py::TestView::test_archive sql/tests.py::TestView::test_workflowsdetail sql/tests.py::TestView::test_workflowsdetail_sqlworkflow_renders_detail sql/tests.py::TestView::test_workflowsdetail_offline_download_renders_sql_detail sql/tests.py::TestView::test_workflowsdetail_missing_audit_id_returns_404 sql/test_archiver.py::test_archive_detail_view` passed.
- `.venv/bin/python -m pytest sql/test_query_privileges.py::test_query_privilege_audit sql/test_archiver.py::test_archive_detail_view sql/tests.py::TestView -k "sqlworkflow or sqlexportworkflow or queryapplylist or archive or workflowsdetail or autoreviewwrong"` passed.
- Broader exploratory pytest command including `sql/test_archiver.py::TestArchiver::test_archive_apply` still fails in setup because `settings.DATABASES["default"]["PORT"]` is an empty string; this occurs before exercising the changed code path.
- Transaction review: `WorkflowContentSerializer.create` saves `SqlWorkflow` and `SqlWorkflowContent` inside the same `transaction.atomic()` as `create_audit()`, so an audit creation exception rolls the new SQL work order back. `query_priv_apply` creates its unsaved `QueryPrivilegesApply` through `create_audit()` inside `transaction.atomic()`, so failure rolls the insert back. `archive_apply` also creates its unsaved `ArchiveConfig` through `create_audit()` inside the surrounding transaction, so failure does not leave a persisted archive work order. SQL review/pre-check exceptions happen before work order save.
- `.venv/bin/python -m black --check sql/utils/workflow_audit.py sql/query_privileges.py sql/archiver.py sql_api/serializers.py sql/tests.py sql/utils/test_workflow_audit.py` passed.
