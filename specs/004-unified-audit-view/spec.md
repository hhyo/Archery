...
2. **Given** a supported legacy detail link can be mapped to an audit ID, **When** a user follows that link, **Then** the user is redirected or guided to the unified view without losing context.
3. **Given** a legacy link cannot be mapped safely, **When** a user follows that link, **Then** the user sees a clear explanation and no unrelated work order data is shown.
