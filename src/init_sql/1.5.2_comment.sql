ALTER TABLE aliyun_access_key COMMENT '阿里云认证信息',
...
ALTER TABLE data_masking_columns COMMENT '脱敏字段配置',
...
ALTER TABLE data_masking_rules COMMENT '脱敏规则配置',
MODIFY `rule_type` INT ( 11 ) NOT NULL COMMENT '规则类型',
...
ALTER TABLE mysql_slow_query_review COMMENT '慢日志统计',
...
MODIFY `comments` LONGTEXT COMMENT '备注'； ALTER TABLE mysql_slow_query_review_history COMMENT '慢日志明细',
...
ALTER TABLE param_history COMMENT '实例参数修改历史-可在线修改的动态参数配置',
MODIFY `instance_id` INT ( 11 ) NOT NULL COMMENT '实例ID',
...
ALTER TABLE param_template COMMENT '实例参数模板配置',
...
ALTER TABLE query_log COMMENT '查询日志-记录在线查询sql的日志',
...
ALTER TABLE query_privileges COMMENT '查询权限记录-用户权限关系表',
...
ALTER TABLE query_privileges_apply COMMENT '查询权限申请记录表',
MODIFY `group_id` INT ( 11 ) NOT NULL COMMENT '组ID',
...
ALTER TABLE resource_group COMMENT '资源组管理-资源组',
...
ALTER TABLE resource_group_relations COMMENT '资源组对象管理-资源组关系表（用户与组、实例与组等）',
...
ALTER TABLE sql_config COMMENT '系统配置',
...
ALTER TABLE sql_instance COMMENT '实例配置-各个线上实例配置',
...
ALTER TABLE sql_users COMMENT '用户管理-用户信息',
...
ALTER TABLE sql_workflow COMMENT 'SQL工单—存放各个SQL上线工单的基础内容',
...
ALTER TABLE sql_workflow_content COMMENT 'SQL工单内容-存放各个SQL上线工单的SQL|审核|执行内容，可定期归档或清理历史数据，也可通过 alter table sql_workflow_content row_format=compressed;
...
ALTER TABLE workflow_audit COMMENT '工作流审批列表-工作流审核状态表',
MODIFY `group_id` INT ( 11 ) NOT NULL COMMENT '组ID',
MODIFY `group_name` VARCHAR ( 100 ) NOT NULL COMMENT '组名称',
...
ALTER TABLE workflow_audit_detail COMMENT '工作流审批明细-审批明细表',
...
ALTER TABLE workflow_audit_setting COMMENT '审批流程配置-审批配置表',
MODIFY `group_id` INT ( 11 ) NOT NULL COMMENT '组ID',
MODIFY `group_name` VARCHAR ( 100 ) NOT NULL COMMENT '组名称',
...
ALTER TABLE workflow_log COMMENT '工作流日志',
