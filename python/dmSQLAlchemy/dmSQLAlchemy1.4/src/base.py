...
from .types import DOUBLE, _DMNumeric
...
class DMCompiler(compiler.SQLCompiler):
    ...
    def limit_clause(self, select, **kw):
        ...
        if select._fetch_clause is not None:
            text += "\n FETCH FIRST %s ROWS" % self.process(select._fetch_clause, **kw)
        return text
        ...
        if lastrowid is not None:
            ...
            if identity_flag:
                ...
                if need_get_primary_flag:
                    ...
                    if len(primary_columns) == 1 and primary_columns[0].type.python_type == int:
                        self.inserted_primary_key = [self.compiled._inserted_primary_key_from_lastrowid_getter(result[0], temp_params)]
                    else:
